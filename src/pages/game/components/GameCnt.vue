<template>
  <div
    id="gameCnt"
    class="cnt h-full relative grid gap-2 grid-cols-4"
    ref="cnt"
  >
    <!-- 展示底部格子 -->
    <div
    ...
    <div
      v-show="!lazyShow"
      v-for="(item, index) in valLists"
      class="val rounded-md absolute flex justify-center items-center p-2"
      :class="item.val ? getClass(item.val) : 'hide'"
      :style="{
        transform: `translate(${
          (item.moveIndex % rowLen) * (itemWidth + 10)
        }px, ${
          parseInt(item.moveIndex / rowLen) * (1 / rowLen) * cntWidth
        }px) `,
        width: itemWidth + 'px',
        height: itemWidth + 'px',
      }"
...
const isGameOver = () => {
  ...
  if (valLists.value.every((item) => item.val !== 0)) {
    ...
    for (let i = 0; i < rowLen * rowLen; i++) {
      ...
      if (
        topInxdex > -1 &&
        valLists.value[i].val === valLists.value[topInxdex].val
      ) {
        noSame = false;
        break;
      }
      ...
      if (
        botInxdex < valLists.value.length &&
        valLists.value[i].val === valLists.value[botInxdex].val
      ) {
        noSame = false;
        break;
      }
      ...
      if (
        leftInxdex > -1 &&
        parseInt(i / rowLen) === parseInt(leftInxdex / rowLen) &&
        valLists.value[i].val === valLists.value[leftInxdex].val
      ) {
        noSame = false;
        break;
      }
      ...
      let rightInxdex = i + 1;
      if (
        rightInxdex < valLists.value.length &&
        parseInt(i / rowLen) === parseInt(rightInxdex / rowLen) &&
        valLists.value[i].val === valLists.value[rightInxdex].val
      ) {
        noSame = false;
        break;
      }
